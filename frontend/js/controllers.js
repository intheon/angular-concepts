		...
		NgMap.getMap().then((map) => {
			...
			$scope.toggleEditing = () => {
				...
				if (!$scope.isEditing)
				{
					...
					miscHelpFunctionsService.displayErrorMessage("Click or tap on the map to add a new park!");
			...
			google.maps.event.addListener(inst.map, "click", (event) => {
				...
				if (!$scope.isEditing) return false;
